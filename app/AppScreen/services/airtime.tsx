...
import { SafeAreaView, StyleSheet, Text, Platform, Alert,
    TouchableWithoutFeedback, KeyboardAvoidingView, Keyboard, TextInput, View } from "react-native";
...
export default function Airtime() {
    ...
    const [cancelBottomOverLay, setCancelBottomOverLay] = useState<boolean>(false)
    ...
    const rechargeCheckHandler = () => {
        if(Number(amount) < 50) {
            Alert.alert("Amount must be between ₦50 and ₦500,000")
            return;
        }
        setCancelBottomOverLay(true)
    }
        ...
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding": "height"}>
            <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
                <SafeAreaView>
                    ...
                    {cancelBottomOverLay && <BottomOverLay 
                        amount={amount} 
                        bonus={0} //cashback Bonus to Earn in number
                        number={Number(phoneNumber)} 
                        networkType="Airtime"
                        recharge="Recharge"
                        networkLogo={"network logo here later"}
                        network={networkSelected}
                        closeBottomOverLay={() => setCancelBottomOverLay(false)}
                    />}
                </SafeAreaView>
